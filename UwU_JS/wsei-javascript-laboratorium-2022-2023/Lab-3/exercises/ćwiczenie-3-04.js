...
function parseStudents(input) {
    ...
    input = input.split('\n').map(object => {
        object = object.split('\t');
            ...
            return {name: object[0],
                    ects: +object[1],
                    active: object[2] == 'aktywny'|| object[2] == 'aktywna' 
            }
            
        })
       
    input.forEach(element => {
        if(element.name ===null || element.name ==='' || element.name >25) input.pop(element);
        if( element.ects >200 ) input.pop(element);
    });
    return input;
...
students = parseStudents(studentLines);
